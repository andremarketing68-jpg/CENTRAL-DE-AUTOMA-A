...
def copiar_para_clipboard(texto):
    """Função nativa do Windows para injetar o texto direto no Ctrl+V"""
...
# --- DICIONÁRIO DE ROBÔS JAVASCRIPT ---
SCRIPTS_JAVASCRIPT = {
    "TRE": """(async function () {
        ...
        for (let i = 0; i < uniqueCodes.length; i++) {
            ...
            if (addBtn) addBtn.click(); await delay(1500); inputField.value = "";
        }
        alert("Concluído! Foram inseridos " + uniqueCodes.length + " códigos únicos.");
    })();""",

    "MEDSENIOR": """(function () {
        var P = document.getElementById('painel-v60-27'); if (P) P.remove();
        var painel = document.createElement('div'); painel.id = 'painel-v60-27';
        ...
        var log = msg => document.getElementById('statusLog').innerText = msg; var winAlvo = null;
        function unlock() {
            try {
                document.body.style.cursor = 'default'; document.body.style.pointerEvents = 'auto';
                var wins = [window, window.top]; if (winAlvo) wins.push(winAlvo);
                wins.forEach(w => {
                    if (w.document) {
                        w.document.body.style.cursor = 'default'; w.document.body.style.pointerEvents = 'auto';
        ...
        function findBtn(w) {
            try {
                var b = w.document.getElementById('button2'); if (b) return { btn: b, win: w };
                if (w.frames) { for (var i = 0; i < w.frames.length; i++) { var r = findBtn(w.frames[i]); if (r) return r; } }
            } catch (e) {} return null;
        }
        document.getElementById('btnRun').onclick = function () {
            var txt = document.getElementById('txtInput').value; var raw = txt.match(\/\\\\b\\\\d{8}\\\\b\/g); if (!raw) return alert('Sem códigos!');
            var counts = {}; raw.forEach(x => counts[x] = (counts[x] || 0) + 1); var unicos = [...new Set(raw)];
            var order = unicos.filter(c => counts[c] === 1).concat(unicos.filter(c => counts[c] > 1)); var codigos = order.map(k => ({ cod: k, qtd: counts[k] }));
            var res = findBtn(window.top); if (!res) return alert('Botão button2 sumiu!'); winAlvo = res.win;
            document.getElementById('btnRun').disabled = true; log('Iniciando...'); var idx = 0;
            function aguardarResp(cb) {
                let t = 0; let c = setInterval(() => {
                    let bl = false; try { if (winAlvo && winAlvo.document.querySelector('.blockUI, .ui-widget-overlay, .ajax-status')) bl = true; } catch (e) {}
                    if (!bl) { clearInterval(c); cb(); } else { t++; if (t > 40) { clearInterval(c); unlock(); cb(); } }
                }, 250);
            }
            function loop() {
                if (idx >= codigos.length) { unlock(); log('✅ FIM!'); document.getElementById('btnRun').disabled = false; return; }
                aguardarResp(function () {
                    var item = codigos[idx]; var code = item.cod; var q = item.qtd;
                    log('Ln ' + (idx + 1) + ': ' + code + (q > 1 ? ' Qtd: ' + q : '')); res.btn.click();
                    var idField = 'item_medico_' + (idx + 1); var idQtd = 'qtd_solicitada_' + (idx + 1); var idNome = 'nome_item_proc_' + (idx + 1);
                    var tries = 0; var timer = setInterval(function () {
                        var field = winAlvo.document.getElementById(idField);
                        if (field) {
                            clearInterval(timer); field.focus(); field.value = code;
                            field.dispatchEvent(new Event('input', { bubbles: true })); field.dispatchEvent(new Event('change', { bubbles: true })); field.blur();
                            log('Aguardando nome preencher...'); var nomeTries = 0;
                            var nomeCheck = setInterval(function () {
                                ...
                                if (val.trim().length > 2) { clearInterval(nomeCheck); log('Nome OK!'); processarQtd(); }
                                else { nomeTries++; if (nomeTries > 60) { clearInterval(nomeCheck); log('Aviso: Timeout Nome'); processarQtd(); } }
                            }, 250);
                            function processarQtd() {
                                if (q > 1) {
                                    var qTries = 0; var qCheck = setInterval(function () {
                                        var qField = winAlvo.document.getElementById(idQtd);
                                        if (qField) {
                                            clearInterval(qCheck); qField.focus(); qField.value = q;
                                            qField.dispatchEvent(new Event('input', { bubbles: true })); qField.dispatchEvent(new Event('change', { bubbles: true })); qField.blur();
                                            idx++; setTimeout(loop, 100);
                                        } else { qTries++; if (qTries > 20) { clearInterval(qCheck); log('Aviso: Qtd falhou na linha ' + (idx + 1)); idx++; setTimeout(loop, 100); } }
                                    }, 250);
                                } else { idx++; setTimeout(loop, 100); }
                            }
                        } else { tries++; if (tries > 50) { clearInterval(timer); if (confirm('Campo ' + idField + ' não abriu. Pular?')) { idx++; loop(); } else { log('Parado.'); document.getElementById('btnRun').disabled = false; } } }
                    }, 100);
                });
            } loop();
        };
    ...
    "TJDF": """(() => {
        ...
        let codigos = []; let idx = 0; let observer; let obsTabelaAtual = null; let executando = false; let pausado = false; let painel = null; let statusEl, contadorEl;
        const criarPainelEntrada = () => {
            painel = document.createElement('div'); painel.id = 'b403-painel-root';
            ...
            document.body.appendChild(painel); painel.querySelector('#b403-iniciar').onclick = iniciarAutomacao;
        };
        const iniciarAutomacao = () => {
            const texto = painel.querySelector('#b403-input').value || ''; const matches = texto.match(\/403\\\\d{5}\/g) || [];
            if (!matches.length) { alert('Nenhum código válido.'); return; }
            const contagem = {}; matches.forEach(m => { contagem[m] = (contagem[m] || 0) + 1; }); const unicos = [...new Set(matches)];
            const order = unicos.filter(c => contagem[c] === 1).concat(unicos.filter(c => contagem[c] > 1)); codigos = order.map(k => ({ cod: k, qtd: contagem[k] }));
            ...
            statusEl = painel.querySelector('#b403-status'); contadorEl = painel.querySelector('#b403-contador');
            painel.querySelector('#b403-pausar').onclick = togglePause; painel.querySelector('#b403-pular').onclick = () => { executando = false; avancarProximo(); }; painel.querySelector('#b403-encerrar').onclick = finalizar;
            observer = new MutationObserver(() => !pausado && executarProximo()); observer.observe(document.body, { childList: true, subtree: true }); executarProximo();
        };
        const setStatus = t => statusEl.textContent = 'Status: ' + t; const setContador = () => contadorEl.textContent = idx + ' / ' + codigos.length;
        const togglePause = () => { pausado = !pausado; setStatus(pausado ? 'pausado' : 'retomado'); if (!pausado) executarProximo(); };
        const adicionarEventoEnterAoInput = () => {
            const input = document.querySelector('#HandleTermo'); if (!input || input.dataset.enterAdded) return;
            input.addEventListener('paste', () => { setTimeout(() => { input.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', keyCode: 13, which: 13, bubbles: true })); }, 100); });
            input.dataset.enterAdded = '1';
        };
        const selecionarTabelaTJDF = () => {
            setStatus('aguardando tabela'); obsTabelaAtual = new MutationObserver(() => {
                const celula = document.querySelector('#result-body-table > tr.dataGridRow.ng-scope.kb-active > td:nth-child(2)');
                if (celula) { celula.click(); obsTabelaAtual.disconnect(); obsTabelaAtual = null; verificarEPreencherQuantidade(); }
        ...
        const verificarEPreencherQuantidade = () => {
            ...
            if (itemAtual.qtd > 1) {
                setStatus('preenchendo qtd (' + itemAtual.qtd + ')'); let tentativas = 0;
                const checarInput = setInterval(() => {
                    ...
                    if (inputQtd) { clearInterval(checarInput); inputQtd.value = itemAtual.qtd; inputQtd.dispatchEvent(new Event('input', { bubbles: true })); inputQtd.dispatchEvent(new Event('change', { bubbles: true })); avancarProximo(); }
                    else { tentativas++; if (tentativas > 20) { clearInterval(checarInput); console.warn('Campo de quantidade não apareceu a tempo.'); avancarProximo(); } }
                }, 500);
            } else { avancarProximo(); }
        };
        const avancarProximo = () => { executando = false; idx++; executarProximo(); };
        const executarProximo = () => {
            if (pausado || executando) return; if (idx >= codigos.length) { finalizar(); return; }
            const c = document.querySelector('#HandleTermo'); if (!c) return; executando = true; setStatus('processando'); setContador();
            adicionarEventoEnterAoInput(); selecionarTabelaTJDF(); c.focus(); c.value = codigos[idx].cod;
            c.dispatchEvent(new Event('paste', { bubbles: true })); c.dispatchEvent(new Event('input', { bubbles: true })); c.dispatchEvent(new Event('change', { bubbles: true }));
        };
        const finalizar = () => {
            pausado = true; executando = false; if (observer) observer.disconnect(); if (obsTabelaAtual) obsTabelaAtual.disconnect(); setStatus('finalizado');
            document.querySelectorAll('.modal-backdrop').forEach(b => b.remove());
            document.querySelectorAll('.modal').forEach(m => { m.style.display = 'none'; m.classList.remove('in', 'show'); });
            document.body.classList.remove('modal-open'); document.body.style.pointerEvents = 'auto'; document.body.style.overflow = 'auto';
            const btnFechar = document.createElement('button'); btnFechar.textContent = '🧹 Fechar painel'; btnFechar.style = 'margin-top:10px;width:100%;padding:8px;border:none;border-radius:8px;background:#444;color:#fff;cursor:pointer;';
            btnFechar.onclick = () => { painel.remove(); painel = null; }; painel.appendChild(btnFechar);
        };
        criarPainelEntrada();
    })();""",

    "SULAMERICA": """(function () {
        const TEMPO = 150; var texto = prompt("MODO SONIC: Cole os códigos aqui:"); if (!texto) return;
        var raw = texto.match(\/\\\\b403\\\\d{5}\\\\b\/g); if (!raw || raw.length === 0) return;
        var counts = {}; raw.forEach(x => counts[x] = (counts[x] || 0) + 1); var unicos = [...new Set(raw)];
        var order = unicos.filter(c => counts[c] === 1).concat(unicos.filter(c => counts[c] > 1)); var codigos = order.map(k => ({ cod: k, qtd: counts[k] }));
        var selInput = "#formValidaProcedimento > fieldset > div > div > div:nth-child(1) > input"; var selBtn = "#btn-incluir-procedimento > span"; var selQtd = "#tabelaSolicitaProcedimento > tbody > tr > td:nth-child(5) > input";
        async function run() {
            for (let i = 0; i < codigos.length; i++) {
                let item = codigos[i]; let inpt = document.querySelector(selInput); let btn = document.querySelector(selBtn);
                if (inpt && btn) {
                    inpt.value = item.cod; inpt.dispatchEvent(new Event('input', { bubbles: true })); inpt.dispatchEvent(new Event('change', { bubbles: true })); btn.click();
                    await new Promise(r => setTimeout(r, TEMPO));
                    if (item.qtd > 1) {
                        await new Promise(r => setTimeout(r, 150)); let qInps = document.querySelectorAll(selQtd); let qInpt = qInps[i] || qInps[qInps.length - 1];
                        if (qInpt) { qInpt.value = item.qtd; qInpt.dispatchEvent(new Event('input', { bubbles: true })); qInpt.dispatchEvent(new Event('change', { bubbles: true })); }
                    }
                }
    ...
    "TST": """(function () {
        var b = document.createElement("button"); b.innerText = "⚖️ ROBÔ EQUILIBRADO (TUSS 16)";
        b.style = "position:fixed;top:10px;left:50%;transform:translateX(-50%);padding:15px;background:#008b8b;color:white;font-weight:bold;border:3px solid white;z-index:9999999;box-shadow:0 0 10px rgba(0,0,0,0.5);";
        document.body.appendChild(b); alert("Robô TST Injetado!");
    })();"""
}

# --- MENU PRINCIPAL DA CENTRAL (EM PYTHON) ---
def menu_central():
    # Envia sinal de máquina ativa ao Firebase
    avisar_firebase_online()
    
    while True:
        os.system('cls' if os.name == 'nt' else 'clear')
        print("="*60)
        print("          ⚙️  CENTRAL DE AUTOMAÇÃO DE FATURAMENTO          ")
        print("="*60)
        print(" Selecione a operadora para COPIAR o script:")
        print(" " + "-"*58)
        print(" [1] Robô TRE")
        print(" [2] Robô MEDSENIOR")
        print(" [3] Robô TJDF")
        print(" [4] Robô SULAMERICA")
        print(" [5] Robô PM/STJ (Usa o mesmo motor do TJDF)")
        print(" [6] Robô TST")
        print(" [0] Sair da Central")
        print("="*60)
        ...
        mapeamento = {
            "1": ("TRE", "TRE"),
            "2": ("MEDSENIOR", "MedSenior"),
            "3": ("TJDF", "TJDF"),
            "4": ("SULAMERICA", "SulAmérica"),
            "5": ("TJDF", "PM/STJ"), # Mapeado para o código estruturado do TJDF
            "6": ("TST", "TST")
        }
        
        if opcao in mapeamento:
            chave_script, nome_robo = mapeamento[opcao]
            script_js = SCRIPTS_JAVASCRIPT[chave_script]
            
            # Executa a cópia para a área de transferência
            copiar_para_clipboard(script_js)
            
            print("\n" + "*"*50)
            print(f" ✅ CÓDIGO DO ROBÔ [{nome_robo.upper()}] COPIADO!")
            print("*"*50)
            print(" -> Vá até o portal da operadora no navegador.")
            print(" -> Abra o Console (Aperte F12).")
            print(" -> Pressione Ctrl + V e aperte ENTER para iniciar.")
            print("*"*50)
...
if __name__ == "__main__":
    ...
    "TST": () => {
        (function () {
            ...
            b.innerText = "Resumo ROBÔ EQUILIBRADO (TUSS 16)";
    ...
    "TRF": () => {
        (function () {
            ...
            s.textContent = `
                ...
                var loopTRF = setInterval(function() {
                    if (!winMain || winMain.closed) {
                        document.getElementById('msg').innerText = "Janela principal fechada!";
                        clearInterval(loopTRF);
                        return;
                    }
                    
                    if (idx >= lista.length) {
                        document.getElementById('msg').innerHTML = "<b style='color:green'>FIM DO PROCESSO TRF!</b>";
                        document.getElementById('status').innerText = "Todos os códigos inseridos com sucesso.";
                        clearInterval(loopTRF);
                        return;
                    }
                    
                    try {
                        var doc = winMain.document;
                        var inp = doc.querySelector(selCod);
                        
                        if (inp && inp.value === "") {
                            var atual = lista[idx];
                            var qtdAtual = qtds[atual];
                            
                            document.getElementById('msg').innerText = "Lançando: " + atual + " (" + (idx + 1) + "/" + lista.length + ")";
                            document.getElementById('status').innerText = "Quantidade definida: " + qtdAtual;
                            
                            // Preenche a tabela se necessário
                            var tab22 = doc.querySelector(sel22);
                            if (tab22 && tab22.value === "") {
                                tab22.value = "22";
                                tab22.dispatchEvent(new Event('change', { bubbles: true }));
                            }
                            
                            // Insere código principal
                            inp.focus();
                            inp.value = atual;
                            inp.dispatchEvent(new Event('input', { bubbles: true }));
                            inp.dispatchEvent(new Event('change', { bubbles: true }));
                            ...
                            // Insere quantidade
                            var campoQtd = doc.querySelector(selQtd);
                            if (campoQtd) {
                                campoQtd.value = qtdAtual;
                            ...
                            setTimeout(function() {
                                var btnSalvar = doc.querySelector(selBtnSalvar);
                                if (btnSalvar) {
                                    btnSalvar.click();
                                    idx++;
                                }
                        ...
                        document.getElementById('status').innerText = "Aguardando carregamento dos elementos do formulário...";
                    }
                }, 1800);
            `;
            W.document.body.appendChild(s);
